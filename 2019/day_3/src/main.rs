...
fn get_wire_points(wire: Vec<&str>) ->  HashMap<String, Point> {
    let  mut point_vector_hashed = HashMap::new();
    let mut x: i32 = 0;
    let mut y: i32 = 0;
    let mut steps: u32 = 0;
    ...
    for part in &wire {
        ...
        for _ in 0..part_steps {
            ...
            steps += 1;
            let s = format!("{},{}", x, y);
            point_vector_hashed.insert(s, Point { x, y, steps });
        }
    }
    point_vector_hashed
}
